SUBROUTINE TEST_ASSERT_NEQ_PASS_INT_1BYTE
    USE FORTUTF
    INTEGER(INT_KIND_1BYTE) :: X, Y
    X = 2
    Y = 4

    CALL TAG_TEST("TEST_ASSERT_NEQ_PASS_INT_1BYTE")

    ! As this test should pass we can leave the
    ! exit code for the framework as is

    CALL ASSERT_NOT_EQUAL(X, Y)

END SUBROUTINE TEST_ASSERT_NEQ_PASS_INT_1BYTE

SUBROUTINE TEST_ASSERT_NEQ_PASS_INT_2BYTE
    USE FORTUTF
    INTEGER(INT_KIND_2BYTE) :: X, Y
    X = 2
    Y = 4

    CALL TAG_TEST("TEST_ASSERT_NEQ_PASS_INT_2BYTE")

    ! As this test should pass we can leave the
    ! exit code for the framework as is

    CALL ASSERT_NOT_EQUAL(X, Y)

END SUBROUTINE TEST_ASSERT_NEQ_PASS_INT_2BYTE

SUBROUTINE TEST_ASSERT_NEQ_PASS_INT_4BYTE
    USE FORTUTF
    INTEGER(INT_KIND_4BYTE) :: X, Y
    X = 2
    Y = 4

    CALL TAG_TEST("TEST_ASSERT_NEQ_PASS_INT_4BYTE")

    ! As this test should pass we can leave the
    ! exit code for the framework as is

    CALL ASSERT_NOT_EQUAL(X, Y)

END SUBROUTINE TEST_ASSERT_NEQ_PASS_INT_4BYTE

SUBROUTINE TEST_ASSERT_NEQ_PASS_INT_8BYTE
    USE FORTUTF
    INTEGER(INT_KIND_8BYTE) :: X, Y
    X = 2
    Y = 4

    CALL TAG_TEST("TEST_ASSERT_NEQ_PASS_INT_8BYTE")

    ! As this test should pass we can leave the
    ! exit code for the framework as is

    CALL ASSERT_NOT_EQUAL(X, Y)

END SUBROUTINE TEST_ASSERT_NEQ_PASS_INT_8BYTE

SUBROUTINE TEST_ASSERT_NEQ_FAIL_INT_1BYTE
    USE FORTUTF
    INTEGER(INT_KIND_1BYTE) :: X, Y
    X = 4
    Y = 4

    CALL TAG_TEST("TEST_ASSERT_NEQ_FAIL_INT_1BYTE")

    CALL ASSERT_NOT_EQUAL(X, Y)

    CALL RESET_LAST_TEST_STATUS

END SUBROUTINE TEST_ASSERT_NEQ_FAIL_INT_1BYTE

SUBROUTINE TEST_ASSERT_NEQ_FAIL_INT_2BYTE
    USE FORTUTF
    INTEGER(INT_KIND_2BYTE) :: X, Y
    X = 4
    Y = 4

    CALL TAG_TEST("TEST_ASSERT_NEQ_FAIL_INT_2BYTE")

    CALL ASSERT_NOT_EQUAL(X, Y)

    CALL RESET_LAST_TEST_STATUS

END SUBROUTINE TEST_ASSERT_NEQ_FAIL_INT_2BYTE

SUBROUTINE TEST_ASSERT_NEQ_FAIL_INT_4BYTE
    USE FORTUTF
    INTEGER(INT_KIND_4BYTE) :: X, Y
    X = 4
    Y = 4

    CALL TAG_TEST("TEST_ASSERT_NEQ_FAIL_INT_4BYTE")

    CALL ASSERT_NOT_EQUAL(X, Y)

    CALL RESET_LAST_TEST_STATUS

END SUBROUTINE TEST_ASSERT_NEQ_FAIL_INT_4BYTE

SUBROUTINE TEST_ASSERT_NEQ_FAIL_INT_8BYTE
    USE FORTUTF
    INTEGER(INT_KIND_8BYTE) :: X, Y
    X = 4
    Y = 4

    CALL TAG_TEST("TEST_ASSERT_NEQ_FAIL_INT_8BYTE")

    CALL ASSERT_NOT_EQUAL(X, Y)

    CALL RESET_LAST_TEST_STATUS

END SUBROUTINE TEST_ASSERT_NEQ_FAIL_INT_8BYTE


